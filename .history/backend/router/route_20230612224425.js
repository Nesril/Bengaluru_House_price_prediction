...
router.put("/logOut/:userId",async(req,res)=>{
  ...
  if(userId){
     ...
     if(user){
         ...
         let loggedOutUser=await User.findOne({email:email})
         res.status(200).json({success:true,msg:"successfully logged out",data:loggedOutUser,token:generalTokn(loggedUser._id)})
        }
