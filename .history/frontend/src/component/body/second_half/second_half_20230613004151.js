...
export default function Second_half({setUserData,userData}) {
 ...
 let navigate=useNavigate()
  ...
  const onFinish=(data)=>{
    ...
    axios.put(`http://localhost:1994/predict/makeHistor/${userData?.data?._id}`,stringfyValue,
    {
       headers: {
  ...
  return (
    <div className='secon_half'>
      ...
      <div className='predictionValue'>
         <div className='predictionValue-error'>{errorMesssage}</div>
         <div className='predictionValue-text'>  {predictionValue?.prediction}</div> 
      </div>
    </div>
  )
}
