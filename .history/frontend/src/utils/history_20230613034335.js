...
const deleteAll=()=>{
  ...
  axios.get(`http://localhost:1994/predict/deleteHistoryAll/${userData?.data?._id}`,
  {
     headers: {
        authorization:userData?.token,
        'Content-Type': 'application/json'
            ...
            {errorMesssage?<div className='history-error'>{errorMesssage}</div>:
              <div className='history-all'>
                ...
                <div className='history-deletAllButton'>
                    <div>{loadingToDeleteAll?<CircularProgress/>:<Button danger onClick={deleteAll}>Delete All</Button>}</div>
