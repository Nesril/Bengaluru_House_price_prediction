...
router.put("/deleteHistoryAll/:userId",async(req,res)=>{
...
router.put("/deleteHistorySingle/:userId",async(req,res)=>{
  let {userId}=req.params
  ...
  if(userId&&History){
     ...
     if(user){
         ...
         console.log(user);
         console.log(History);
         res.status(200).json({success:true,data:user.history,msg:"history sucessfully deleted"})
      }
     else res.status(401).send({msg:"user not found"})
  }
  else res.status(401).send({msg:"please enter userid"})
 })
...
router.put("/makeHistory/:userId",async (req,res)=>{
    ...
    const date=new Date()
    let value= ""
    console.log(date,area_type,balcony,bath,bhk,location,sqft);
    const user=await User.findById(userId)
    if(user){
       const process = spawn('python', ['backend/router/model.py', location,area_type,parseFloat(sqft),parseInt(bath),parseInt(balcony),parseInt(bhk)]);
       console.log("hy"); 
       ...
       process.stdout.on('data', async (data) => {
            ...
            try{
                ...
                res.status(501).send({error:true,msg:"error occured whiole romaining"})
            }
          });
